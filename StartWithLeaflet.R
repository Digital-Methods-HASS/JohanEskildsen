...
#Q1 Answer: x and y. Longitude first, lattitude last
...
#Q2 Answer: It gives you the option of closing the layer control option
...
l_aus <- leaflet() %>%   # assign the base location to an object
  setView(151.2339084, -33.85089, zoom = 12)
l_aus
...
Danmap_layer <- Danmap %>%
  setView(lng=10.82,lat = 55.83,zoom=6) %>% 
  addLayersControl(baseGroups = names(esri),
...
Danmap_display <- Danmap %>% 
  ...
  addMarkers(lng = places$Longitude,
             lat = places$Latitude,
             popup = paste(places$Placename,"<br>" ,places$Description, "<br>", places$Type, places$Notes, "<br>")) %>% 
  addMeasure(
    position = "bottomleft",
    primaryLengthUnit = "meters",
    primaryAreaUnit = "sqmeters",
