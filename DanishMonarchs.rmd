...
library(tidyverse)
kings1 <- read.csv("data/Monark_ark_Danish.csv")

kings2 <- read_csv("data/Monark_ark_Danish.csv")

kings3 <- read.csv2("data/Monark_ark_Danish.csv")

kings4 <- read_csv2( "data/Monark_ark_Danish.csv")
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
We can see that the correct function is the "kings4". Read_ means that it is Tidyverse that runs it. The csv2 means that the separator is ;
...
#It has 11. The "variables" is, in this case, the same as columns
...
kings_reign_duration <- kings %>% 
  filter(!is.na(Era_start) | !is.na(Era_finished)) %>% 
  mutate(kings_reign_duration=Era_finished - Era_start)

kings_reign_duration [12]
...
avg_reign <- kings_reign_duration %>% 
  summarise(avg_reign = mean(as.numeric(kings_reign_duration), na.rm=TRUE))
...
kings_reign_duration %>% 
  filter(kings_reign_duration>19.33929) %>% 
  select(Name,kings_reign_duration)
...
kings_reign_duration %>% 
  arrange(desc(kings_reign_duration))
  
kings_reign_duration %>% 
  mutate(Days = kings_reign_duration * 365)
